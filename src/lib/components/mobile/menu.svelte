...
<Drawer.Root direction="right">
  ...
  <Drawer.Portal>
    ...
    <Drawer.Content
      class="fixed bottom-0 left-0 right-0 mt-24 flex h-full w-full z-50 p-2"
    >
        ...
        <ul class="flex flex-col gap-2">
          {#each NavLinks as link}
            <li class="flex items-center gap-2">
              <Drawer.Close
				class={cn(buttonVariants({ variant: "ghost", size: "icon" }), "text-neutral-900 border border-neutral-50 w-full px-4 py-2 hover:bg-neutral-50/40")}
